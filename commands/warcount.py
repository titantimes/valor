...
async def _register_warcount(valor: Valor):
    ...
    clone_map = {"Hunter": "Archer", "Knight": "Warrior", "Dark Wizard": "Mage", "Ninja": "Assassin",
                 "Skyseer": "Shaman"}
    real_classes = clone_map.values()
    season_times = {8: [1663016400, 1666569600],
                    9: [1666983600, 1672462800]}  # ! idk the end time for season 9 so i put a big number
    ...
    async def warcount(ctx: Context, *options):
        try:
            ...
            return await LongTextEmbed.send_message(valor, ctx, "warcount",
                                                    parser.format_help().replace("main.py", "-warcount"), color=0xFF00)

        if not opt.range:
            warcount_nonrange(opt, ctx)
        else:
            warcount_range(opt, ctx)

    def warcount_range(opt, ctx):
        counts = []
        ...
        for doc in collection.find({"_id": {"$gt": t0, "$lt": t1}}):
            if names != "Anything" and not doc["sender"].lower() in names:
                continue
            elif names != "Anything":
        ...
        content = "```\n" + header + '\n' + sep + '\n' + '\n'.join(
            format % tuple(map(str, count)) for count in counts) + "```"

        await LongTextEmbed.send_message(valor, ctx, "Warcount", content=content,
                                         color=0x11FFBB,
                                         footer=footer
                                         )

    def warcount_nonrange(opt, ctx):
        counts = []
